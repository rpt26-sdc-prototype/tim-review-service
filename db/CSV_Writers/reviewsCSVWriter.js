...
(async () => {
  ...
  try {
    ...
    await PromiseX.each([...new Array(_reviewsBigBatchNumber)], async (bigBatch, dex, arrayLength) => {
      ...
      fs.writeFileSync("oneMillionReviews.csv", '');
      const test = fs.readFileSync('oneMillionReviews.csv').toString();
      console.log(test);
      const stream = fs.createWriteStream("oneMillionReviews.csv", { flags: 'a' });
      ...
      stream.end();
      let reviewsEntryValues = fs.readFileSync(`${__dirname}/oneMillionReviews.csv`);
